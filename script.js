window.onload = function () {
  ...
  function gameOver() {
    ...
    ctx.strokeText("Score " + score.toString(), centreX, centreY);
  ...
  function drawScore() {
    ...
    ctx.fillText("Score : " + score.toString(), centreX, canvasHeight - 20);
    ctx.restore();
  }

  function refreshCanvas() {
    snakou.advance();
    if (snakou.checkCollision()) {
      gameOver();
    } else {
      if (snakou.isEatingApple(applou)) {
        //Le serpent à manger la pomme
        score++;
        snakou.ateApple = true;
        do {
          applou.setNewPosition();
        } while (applou.isOnSnake(snakou))
      }
      ctx.clearRect(0, 0, canvasWidth, canvasHeight - 1);
      drawScore();
      snakou.draw();
      applou.draw();
  ...
  document.onkeydown = function handleKeyDown(e) {
    ...
    snakou.setDirection(newDirection);
  };
};
